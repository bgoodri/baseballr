...
scrape_statcast_savant_pitcher <- function(start_date, end_date, pitcherid) {
    ...
    tryCatch(
        {
            ...
            payload <- utils::read.csv(url, na.strings = "null", stringsAsFactors = FALSE)
    ...
    payload$barrel <- with(payload, ifelse(launch_angle <= 50 & launch_speed >= 98 & launch_speed * 1.5 - launch_angle >= 11 & launch_speed + launch_angle >= 124, 1L, 0L))
